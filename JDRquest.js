...
window.addEventListener("load", async () => {
  ...
  Object.entries(playerJSON).forEach(([index, player]) => {
...
function chooseEnemy(category = undefined) {
  // prettier-ignore
...
function initializeEnemy(enemyData) {
  if (!enemyData) return;
  const rarityE = document.querySelector("#rarity");
  if (rarity === 1) {
